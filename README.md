...
Generate Atom feeds from websites that lack them. Define each site's structure with CSS selectors in a YAML config file; xml-feed scrapes the index page, extracts post content, and produces a valid Atom XML feed.
...
```sh
go install github.com/jandubois/xml-feed@latest
```
...
```sh
go build -o xml-feed .
cp xml-feed /usr/local/bin/
```

## Usage

```
xml-feed                           # process all configured sites
xml-feed --site myblog             # process one site
xml-feed --limit 10                # only process the 10 newest posts
xml-feed --delay 2s                # wait 2 seconds between fetches
xml-feed --config /path/to/config  # use a different config file
xml-feed --version                 # print version
```
...
Default config path: `~/.config/xml-feed/config.yaml`
...
| Purpose | Path |
|---------|------|
| Config  | `~/.config/xml-feed/config.yaml` |
| Cache   | `~/.cache/xml-feed/<hostname>/` |
| Output  | `~/.local/share/xml-feed/<hostname>.xml` |
